...
Für die Wiederverwendung von Funktionalität kennt die Objektorientierte Programmierung ([OOP](OOP.md)) zwei sehr bekannte Kandidaten: Die Vererbung (whitebox – reuse) und die Komposition (blackbox – reuse). Verwendet man Funktionalität wieder durch das Ableiten von einer Klasse, so ist die Subklasse abhängig von der Elternklasse. Dies macht ein System in vielen Fällen unnötig komplex, schlechter testbar und erschwert das Austauschen von Funktionalität zur Laufzeit. [Clean Code Developer](CleanCode/Clean%20Code%20Developer.md) hat für das korrekte Ableiten das [Liskov Substitution Principle](CleanCode/Liskov%20Substitution%20Principle.md) bereit, das es dabei zu befolgen gilt.
