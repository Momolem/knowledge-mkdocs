...
# Singleton
## Intent
**Singleton** is a [creational design pattern](CleanCode/Design%20Patterns.md#creational) that lets you ensure that a class has **only one instance**, while providing a global access point to this instance.

## Problem
The Singleton pattern solves two problems at the same time, violating the _[Single Responsibility Principle](CleanCode/Single%20Responsibility%20Principle.md)_:
...
| Pros | Cons |
| --- | --- |
| You can be sure that a class has only a single instance. | Violates the _[Single Responsibility Principle](CleanCode/Single%20Responsibility%20Principle.md)_. The pattern solves two problems at the time. |
...
| | It may be difficult to [Unit Test](CleanCode/Unit%20Test.md) the client code of the Singleton because many test frameworks rely on inheritance when producing mock objects. Since the constructor of the singleton class is private and overriding static methods is impossible in most languages, you will need to think of a creative way to mock the singleton. Or just don’t write the tests. Or don’t use the Singleton pattern. |
...
-   [Flyweight](CleanCode/Flyweight.md) would resemble [Singleton](CleanCode/Singleton.md) if you somehow managed to reduce all shared states of the objects to just one flyweight object. But there are two fundamental differences between these patterns:
    1.  There should be only one Singleton instance, whereas a _[Flyweight](CleanCode/Flyweight.md)_ class can have multiple instances with different intrinsic states.
    2.  The _Singleton_ object can be mutable. [Flyweight](CleanCode/Flyweight.md) objects are immutable.
