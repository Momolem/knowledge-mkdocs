...
| The proxy works even if the service object isn’t ready or is not available. ||
 | [Open Closed Principle](CleanCode/Open%20Closed%20Principle.md). You can introduce new proxies without changing the service or clients. ||
 
## Relations with Other Patterns
- [Adapter](CleanCode/Adapter.md) provides a different interface to the wrapped object, Proxy provides it with the same interface, and [Decorator](CleanCode/Decorator.md) provides it with an enhanced interface.
- [Facade](CleanCode/Facade.md) is similar to Proxy in that both buffer a complex entity and initialize it on its own. Unlike [Facade](CleanCode/Facade.md), Proxy has the same interface as its service object, which makes them interchangeable.
- [Decorator](CleanCode/Decorator.md) and Proxy have similar structures, but very different intents. Both patterns are built on the composition principle, where one object is supposed to delegate some of the work to another. The difference is that a Proxy usually manages the life cycle of its service object on its own, whereas the composition of Decorators is always controlled by the client.